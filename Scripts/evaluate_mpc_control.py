...
from Data.cartpole_data import CartPoleGenerator, rollout_and_save_data
from Envs.custom_cartpole_v1 import CustomCartPole, make_cartpole_reward_on_traj
from Models.cnp_model import CNPModel, gen_context
...
if __name__ == '__main__':
    ...
    #     action, expectation = controller.next_action(obs)
    #     obs, rew, done, info = cartpole_env.step(int(action))
    #     accumulated_reward += rew
    #     print(f'Current_reward: {accumulated_reward}', f'Expectation:{expectation}')
    #     cartpole_env.render()
    #     if done:
    #         break

...
from Data.cartpole_data import CartPoleGenerator
...
if __name__ == '__main__':
    ...
    # ------------------- DYNAMICS -------------------- #
    if train_dynamics:
        cartpole_data_gen = CartPoleGenerator(savedir='offline/cartpole', batch_size=16, num_context=[10, 20])
        if gen_new_data:
    ...
    controller = NaiveMPCController(action_space=cartpole_env.action_space,
                                    ...
                                    cost_fn=make_cartpole_reward(cartpole_env),
                                    horizon=100,
    ...
    if mpc_rollout:
        ...
        for i in tqdm(range(200)):
            action = int(controller.get_action(obs)[0])
    ...
    results = {'mf': [], 'mpc-bc': []}
    a2c_trainer = a3c.A2CTrainer(config=config, env='CustomCartPole-v1')
    for i in tqdm(range(100)):
        ...
        results['mf'].append(result['episode_reward_mean'])
        if i % 50 == 0:
            checkpoint = a2c_trainer.save()
            print("checkpoint saved at", checkpoint)
        if i % 50 == 0:
            rollout.rollout(a2c_trainer, env_name='CustomCartPole-v1', num_steps=50, num_episodes=1, no_render=False)
    # ------------------------- Behavior Cloning -------------------------- #
    # Pure A2C
    ray.shutdown(True)
    ray.init(num_gpus=1, )
